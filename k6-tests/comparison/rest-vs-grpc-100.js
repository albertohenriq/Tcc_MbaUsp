// Teste comparativo REST vs gRPC - 100 usuários
...
export const options = {
  stages: [
    ...
    { duration: '30s', target: 0 },    // Redução gradual
  ],
  thresholds: {
    'rest_latency': ['p(95)<500'],    // 95% das requisições REST < 500ms
    'grpc_latency': ['p(95)<500'],    // 95% das requisições gRPC < 500ms
  ...
  const resGRPC = http.post('http://localhost:3000/grpc/process',
    JSON.stringify(payload),
    {
      headers: { 
        ...
        'X-Protocol': 'grpc'  // Header especial para identificar chamadas gRPC
      },
    }
  );
